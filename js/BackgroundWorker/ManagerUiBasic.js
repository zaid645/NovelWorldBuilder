...
export const ManagerUiBasic = {
    ...
    renderSidebar() {
        ...
        if (writeMenuList) {
            writeMenuList.innerHTML = `
                ...
                <button onclick="app.switchView('calendar')" class="w-full text-left px-3 py-2 rounded transition text-sm flex items-center mb-1 ${this.currentView === 'calendar' ? 'bg-indigo-600 text-white font-medium' : 'text-slate-300 hover:bg-slate-700'}">
                    ...
                    <span>Perencanaan Kalender</span>
                </button>
                ...
                <button onclick="app.switchView('chapter-outline')" class="w-full text-left px-3 py-2 rounded transition text-sm flex items-center mb-1 ${this.currentView === 'chapter-outline' ? 'bg-indigo-600 text-white font-medium' : 'text-slate-300 hover:bg-slate-700'}">
                    ...
                    <span>Manajemen Outline Cerita</span>
                </button>
        ...
        if (fantasyMenuList) {
            fantasyMenuList.innerHTML = `
                ...
                <!-- Kelas / Class (BARU DITAMBAHKAN) -->
                <button onclick="app.switchView('classes')" id="menu-classes" class="w-full text-left px-3 py-2 rounded transition text-sm flex items-center ${this.currentView === 'classes' ? 'bg-indigo-600 text-white font-medium' : 'text-slate-300 hover:bg-slate-700'}">
                    <svg class="w-4 h-4 mr-2 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Gelar / Title (BARU DITAMBAHKAN) -->
                <button onclick="app.switchView('titles')" id="menu-titles" class="w-full text-left px-3 py-2 rounded transition text-sm flex items-center ${this.currentView === 'titles' ? 'bg-indigo-600 text-white font-medium' : 'text-slate-300 hover:bg-slate-700'}">
    ...
    switchView(viewId) {
        ...
        } else if (viewId === 'classes') { // BARU DITAMBAHKAN
            titleEl.innerText = "Master Daftar Kelas / Job";
            if (typeof this.renderClassView === 'function') {
                contentArea.innerHTML = this.renderClassView();
            } else if (typeof this.renderClassesView === 'function') {
                contentArea.innerHTML = this.renderClassesView();
            }
        } else if (viewId === 'titles') { // BARU DITAMBAHKAN
            titleEl.innerText = "Master Daftar Gelar / Title";
            if (typeof this.renderTitleView === 'function') {
                contentArea.innerHTML = this.renderTitleView();
            }
